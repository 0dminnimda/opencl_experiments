...
int main(void) {
    ...
    try {
        ...
        std::cerr << "Errors for failed build for all devices from thrown exception"
                  << std::endl;
        for (auto &pair : buildInfo) {
            std::cerr << "Device: " << pair.first.getInfo<CL_DEVICE_NAME>() << std::endl
                      << pair.second << std::endl
    ...
    programStrings.push_back(
        std::pair<const char *, size_t>(kernel1.data(), kernel1.length()));
    programStrings.push_back(
        std::pair<const char *, size_t>(kernel2.data(), kernel2.length()));
    ...
    *anSVMInt = 5;
    ...
    auto vectorAddKernel =
        cl::KernelFunctor<decltype(fooPointer) &, int *, cl::coarse_svm_vector<int> &,
                          cl::Buffer,
                          std::vector<int, cl::SVMAllocator<int, cl::SVMTraitCoarse<>>> &,
                          int, cl::Pipe &, cl::DeviceCommandQueue>(vectorAddProgram,
                                                                   "vectorAdd");
    ...
    cl_int error;
    vectorAddKernel(
        cl::EnqueueArgs(cl::NDRange(numElements / 2), cl::NDRange(numElements / 2)),
        fooPointer, inputA.data(), inputB, outputBuffer, output2, 3, aPipe,
    ...
    for (size_t s : v2) { std::cout << "\t" << s << "\n"; }

    std::cout << "Output:\n";
    for (int i = 1; i < numElements; ++i) { std::cout << "\t" << output[i] << "\n"; }
    std::cout << "\n\n";
    std::cout << "Output2:\n";
    for (auto &e : output2) { std::cout << "\t" << e << "\n"; }
    std::cout << "\n\n";
